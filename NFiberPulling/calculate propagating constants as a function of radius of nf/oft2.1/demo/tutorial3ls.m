...
% case it is ignored.
materials = {1.46147, 1.45804, 'air'};
fibre.materials = materials;
fibre.coreCladdingRatio = 3.3/125; % 3.3/125 for SM 600
...
task.maxmode = 3; % Calculate only the first of all families of modes, i.e.
% HE11, TE01 and TM01
if calculatemodes
    modesTwoClad = buildModes(argument, fibre, task, false);
end
hTwoClad = showModes(modesTwoClad);
set(hTwoClad, 'Visible', 'off') % Make TE curve dashed
set(hTwoClad(1), 'Visible', 'on', 'Color', 'blue')
set(hTwoClad(3), 'Visible', 'on', 'Color', 'red') % Make TE curve dashed
%%
