function [E, H] = modeFieldCirc(lambda, d, n_eff, fibreSpec, task, R, PHI) 
...
if (strcmpi(task.modetype, 'HYBRID'))
    ...
    Ei_r   = @(r,phi) - 1/sqrt(2)*(a1 * besselj(nu-1,u*r/a) + a2 * besselj(nu+1,u*r/a)) ./ besselj(nu,u);
    Ei_phi = @(r,phi) - 1/sqrt(2)*(a1 * besselj(nu-1,u*r/a) - a2 * besselj(nu+1,u*r/a)) ./ besselj(nu,u);
    Ei_z   = @(r,phi) - 1/sqrt(2)*(1i * u) / (a * beta) * besselj(nu,u*r/a) / besselj(nu,u);
    
    % Magnetic field inner (HE/EH)
    Hi_r   = @(r,phi)   1/sqrt(2)*sqrt(eps0/mu0) * (k0 * n_inn^2/beta) * (a3 * besselj(nu-1,u*r/a) - a4 * besselj(nu+1,u*r/a)) ./ besselj(nu,u);
    Hi_phi = @(r,phi) - 1/sqrt(2)*sqrt(eps0/mu0) * (k0 * n_inn^2/beta) * (a3 * besselj(nu-1,u*r/a) + a4 * besselj(nu+1,u*r/a)) ./ besselj(nu,u);
    Hi_z   = @(r,phi) - 1/sqrt(2)*sqrt(eps0/mu0) * (1i * u * F2) / (k0 * a) * besselj(nu,u*r/a) / besselj(nu,u);
    
    % Electric field outer (HE/EH)
    Eo_r   = @(r,phi) -1/sqrt(2)* u / w * (a1 * besselk(nu-1,w*r/a) - a2 * besselk(nu+1,w*r/a)) ./ besselk(nu,w);
    Eo_phi = @(r,phi) - 1/sqrt(2)*u / w * (a1 * besselk(nu-1,w*r/a) + a2 * besselk(nu+1,w*r/a)) ./ besselk(nu,w);
    Eo_z   = @(r,phi) - 1/sqrt(2)*(1i * u) / (a * beta) * besselk(nu,w*r/a) / besselk(nu,w);
    
    % Magnetic field outer (HE/EH)
    Ho_r   = @(r,phi)  1/sqrt(2)* sqrt(eps0/mu0) * (k0 * n_inn^2/beta) * (u / w) * (a5 * besselk(nu-1,w*r/a) + a6 * besselk(nu+1,w*r/a)) / besselk(nu,w);
    Ho_phi = @(r,phi) - 1/sqrt(2)*sqrt(eps0/mu0) * (k0 * n_inn^2/beta) * (u / w) * (a5 * besselk(nu-1,w*r/a) - a6 * besselk(nu+1,w*r/a)) / besselk(nu,w);
    Ho_z   = @(r,phi) - 1/sqrt(2)*sqrt(eps0/mu0) * (1i * u * F2) / (k0 * a) * besselk(nu,w*r/a) / besselk(nu,w);
