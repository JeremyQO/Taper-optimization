...
L0 = 0.43; % size of the flame in mm
vb = 4.0; %velocit y of the burner in mm/s
Lw = 10; %length of the waist in mm
rw = 0.0002; % waist radius in mm
omega=[1.5e-3, 3.8e-3, 7.5e-3]; %taper linear angles in rad [angle2 angle1]
omega_exp = 3.5e-3;
r_exp = 0.006; %initial radius of the exponential
r2 = 0.0362; % transition radius between the two linear parts
r1 = 0.0418;
...
%the exponential.
c_test=1-sqrt(-L0*omega_exp/r_exp+sqrt(1+(L0*omega_exp/r_exp)^2));
...
%%%%%%%%%%%%%%% SUBSECTION E: FIRST LINEAR TAPER ZONE 1 - ANGLE 2 %%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%% SUBSECTION F: Second LINEAR TAPER ZONE 2 - ANGLE 1 %%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%% SUBSECTION F_2: THIRD LINEAR TAPER ZONE 2 - ANGLE 1 %%%%%%%%%%%%%%%%%%%%

%reinitialization of the parameters before starting the new linear section

rwaist(w)=L0*tan(omega(3)/2)+rwaist(w-1);
cn(w-1)=(rwaist(w)-rwaist(w-1))/rwaist(w);
Lue(w)=2*L0*log(1-cn(w-1))/((1-cn(w-1))^2-1);
vf(w-1)=2*vb*(1-(1-cn(w-1))^2)/(1+(1-cn(w-1))^2);
t0(w-1)=L0/vf(w-1)*log((2*vb+vf(w-1))/(2*vb-vf(w-1)));
...
while rwaist(w)<r0;
    rwaist(w+1)=Lue(w)*tan(omega(3))+rwaist(w-1);
    cn(w)=(rwaist(w+1)-rwaist(w))/rwaist(w+1);
    Lue(w+1)=2*L0*log(1-cn(w))/((1-cn(w))^2-1);
    vf(w)=2*vb*(1-(1-cn(w))^2)/(1+(1-cn(w))^2);
    t0(w)=L0/vf(w)*log((2*vb+vf(w))/(2*vb-vf(w)));
...
for j=1:steps
    ...
    zInterp = linspace(min(real(znew(:))),max(real(znew(:))),N);%Equally spaced z-axis
    rnew = interp1(real(znew),rnew, zInterp);
...
if compute_PVT
% Outputs (give some values -> need to enter the Baptiste's software
PullingTime=sum(PVTM(:,1)); %[s]
disp(['Pulling time = ',num2str(PullingTime,4),' s.']);
dt= min(PVTM(:,1)); %[s].
disp(['Minimum segment duration = ', num2str(dt,4),' s.']);
PulledZoneLength = ((steps+1)*L0+Lw)*1e-3; % [m]
disp(['Pulled zone length = ', num2str(PulledZoneLength*1e2,4),' cm.']);
StartPosL=-49-min(cumsum(PVTM(:,4))); % [mm].
disp(['Left motor starting position > ', num2str(StartPosL,3),' mm.']);
StartPosR=-49-min(cumsum(PVTM(:,2))); % [mm].
disp(['Right motor starting position > ', num2str(StartPosR,3),' mm.']);
PosMaxL=StartPosL+ max(cumsum(PVTM(:,4))); % [mm]
disp(['Left motor maximal position = ', num2str(PosMaxL,3),' mm.']);
PosMaxR=StartPosR+ max(cumsum(PVTM(:,2))); % [mm]
disp(['Right motor maximal position = ', num2str(PosMaxR,3),' mm.']);
end
%% Output
if save_
% PullingTime = sum(PVTM(:,1))
    ...
    %% Save
datestr = date;
path = '/home/jeremy/NFiber/NFiberPulling/pvt/';
timeobject=datetime;
mkdir([path datestr]); % create a folder to save the trajectory.
PVTStr=[num2str(hour(timeobject)),'h',num2str(minute(timeobject)),'-PVTM-rw',num2str(rw*1e6),'-Lw',num2str(Lw),'-L0',num2str(L0),'.txt'];
% dlmwrite(['C:\Users\oq\Desktop\Tirage\' datestr '\' PVTStr] ,PVTM ,'newline','pc');
dlmwrite([path datestr '/' PVTStr] ,PVTM ,'newline','pc'); % Add the right directory path.

WorkspaceStr=[datestr,'-PVTM-rw',num2str(rw*1e6),'-Lw',num2str(Lw),'.mat'];
% save(['C:\Users\oq\Desktop\Tirage\' datestr '\Workspace',WorkspaceStr]);
save([path datestr '/Workspace', WorkspaceStr]); % Add the right directory path.

%% Plots
% figure
% plot(vf,'+','color','b'); % [mm].
% title('Amplitude de course de la flamme vs step'); ylabel('Ln (mm)'); xlabel('step');
% grid on;
% figure
% plot(rn*1e6,'x','color','b'); % [micron].
% title('Fiber radius vs step'); ylabel(' rn (micron)'); xlabel('step');
% grid on;
figure
plot(vf,'+','color','b');% [mm].
title('Vitesse de Tirage vs step'); ylabel('vf (mm/s)'); xlabel('step');
grid on;
